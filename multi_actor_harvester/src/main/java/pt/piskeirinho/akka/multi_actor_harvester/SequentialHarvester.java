...
public class SequentialHarvester implements Runnable {
	...
	public void run() {
		...
		for (String repo : Configs.repositoriesURLs) {
			...
			try {
				Iterator<RecordType> listRecordsIterator = harvester.getListRecordsIterator(null, null,
						Configs.SET_SPEC, Configs.METADATA_PREFIX);
			...
			Utils.output("3 - Done harvesting \"" + repo + "\" - it has " + i + " documents",
